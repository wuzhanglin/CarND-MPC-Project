    ...
    public:
        ...
        void operator()(ADvector& fg, const ADvector& vars)
        {
            ...
            for (size_t t = 1; t < N; t++)
            {
                ...
                fg[1 + psi_start + t] = psi1 - (psi0 - v0 / MPC::GetLf() * delta0 * dt);
                fg[1 + v_start + t] = v1 - (v0 + a0 * dt);
                fg[1 + cte_start + t] = cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt));
                fg[1 + epsi_start + t] = epsi1 - ((psi0 - psides0) - v0 / MPC::GetLf() * delta0 * dt);
            }
        }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs)
{
    ...
    // The upper and lower limits of delta are set to -25 to 25 degrees (values in radians)
    for (size_t i = delta_start; i < a_start; i++)
    {
        vars_lowerbound[i] = -0.436332 * GetLf();
        vars_upperbound[i] = 0.43632 * GetLf();
    }
